...
def gerar_grafico(medias_calculadas, moeda, ano):
    mes = list(medias_calculadas.keys())
    mediaCompra = [medias_calculadas[m]["mediaCompra"] for m in mes]
    mediaVenda = [medias_calculadas[m]["mediaVenda"] for m in mes]

    graf.figure(figsize=(10, 5))
    graf.plot(mes, mediaCompra, label='Média Compra', marker='o')
    graf.plot(mes, mediaVenda, label='Média Venda', marker='o')
...
def main():
    ...
    abrevmoedas, nomemoeda = zip(*moedas)  # Descompacta as listas de siglas e nomes
    ...
    while True:
        ...
        # Gerar gráfico
        gerar_grafico(medias_calculadas, moeda, ano)
